...
// Optional PINE auto-inject: when LBS handoff arms, write handle + unlock flags
// to EE RAM so Go Online is selectable without running unlock_go_online.js.
//
// 2026-05-19: DISABLED for real-PS2 compatibility test. PINE doesn't exist on
// real hardware — anything we depend on PINE for breaks on a real PS2 disc.
// The standing project rule (memory/feedback_independent_servers + the user's
// "no client patches" constraint) means injection must be removed eventually.
// Flip PINE_AUTO_INJECT to true to re-enable for diagnostics. If the lobby
// flow regresses after disabling, we identify which of the 5 EE addresses the
// game isn't setting itself, then trace each back to which packet/response
// should write it.
// 2026-05-23: PINE on. Refocus — diagnose the PINE-on flow's natural state
// changes between snapshots (main menu → after HTTP login → second login →
// after Entry) so we can see what the game writes ITSELF that we'd otherwise
// have to trigger from the server side. PINE-off flow doesn't reach the
// second login menu (errors at /lbs_info), so observing the post-PINE
// trajectory is the only way to see what the game's natural code does.
const PINE_AUTO_INJECT = false; // 2026-05-31: server-PINE was hogging the IPC socket and blocking diagnostic probes. Per project_session_handoff_pine_removal — final product shouldn't use PINE.
...
if (PINE_AUTO_INJECT) {
  ...
  try {
    ...
    // 2026-05-23: reverted to the ORIGINAL fast sequence (no inter-step delays,
    // original write ORDER: handle → 0x701F30 → 0x701F28 → 0x29F280 → 0x35A279).
    // The staged 600ms-delay version (used during Option Y) caused the lobby
    // to render black and PCSX2 PINE to freeze post-lobby. The fast single-shot
    // sequence was the known-working order from earlier Codex tests.
    // PINE_WRITES env var controls which writes fire (default: all).
    // Comma-separated list of: handle, arm, state, boot, downstream
    //   handle     = 0x870870 (handle buffer)
    //   arm        = 0x701F30 = 1 (arm flag — hypothesized core trigger)
    //   state      = 0x701F28 = 0x1A5DF (state var)
    //   boot       = 0x29F280 = 1 (boot-record byte)
    //   downstream = 0x35A279 = 1 (downstream flag)
    // Examples:
    //   PINE_WRITES=arm                     → only 0x701F30=1
    //   PINE_WRITES=arm,handle              → arm + handle buffer
    //   PINE_WRITES=arm,state,handle        → three of five
    //   PINE_WRITES=all                     → all five (default)
    // process.env.PINE_WRITES not set → default 'all'. Empty string OR 'none' → no writes.
    ...
    pineAutoInject = async (handle) => {
      if (!handle || handle.length < 1) { console.log('[pine] skip: empty handle'); return; }
...
// Network identity.
// DNS (UniFi router + server/dns_server.js) resolves all three hostnames below
// to HOST_IP. Use hostnames in protocol payloads — embedded parsers expect
// hostname-shaped strings in "domain name" fields. The raw IP is kept only for
// computing the binary hex form in SNAP_HEX_ADDR (which carries the resolved
// IP, not a hostname). See memory/feedback_dns_works.md.
const HOST_IP   = '192.0.2.196';
const SNAP_PORT = 10127;
const APP_HOST  = 'app01.reo.capcom.sf.yav4.com';        // HTTP/mhweb host
const SNAP_HOST = 'snap01.capcom.client.sf.yav4.com';    // SN@P/game host (LBS field 2)
const SELF_HOST = 'stage6.sega.com';                     // public web host
...
function gameStartPage(sessid) {
  // PAL CSV-parser is strict on field count: 4 fields, no extras.
  // The CSV-extra-fields probe (2026-05-14) was conclusively negative:
  // game does NOT import handle from CSV. Keep response shape minimal.
  const csv = [
    'OK',
    sessid,
    `${BASE_URL}/enterareas`,
  ...
  // 3.DAT browser parser is case-sensitive — markers are uppercase in the
  // overlay (file offset 0x9b820+). Lowercase = parser never matches.
  return `<html>
<head><!--CRS-GAME-START-->
<META HTTP-EQUIV=Content-Type CONTENT=text/html;CHARSET=iso-8859-1>
</head>
...
// 2026-05-23: response for completion of account signup. Returns the
// AM-USA-COMP-SIGNUP marker (ID 99) which fires netwk's handler at 0x5e75a0,
// setting struct+0x192 = 1. That flag is the trigger for the natural field
// commit chain at 0x5e6b08 which reads form-captured fields (login name,
// password) and propagates them to permanent buffers (including the handle
// buffer at 0x870870 we have been writing with PINE).
// HYPOTHESIS: returning this marker on successful /create.jsp POST replaces
// the need for PINE injection of the handle string. The game's natural code
// commits the typed login name itself.
...
function enterAreasPage(sessid) {
  // 2026-05-22 E2: /enterareas returns CRS-GAME-START with start_url=/lbs_info
  // (4-field gameStartPage shape). Hypothesis: the browser needs to navigate
  // through /lbs_info before consuming LBS data — CRS-LBS-INFO-GET handled
  // directly at /enterareas (the E1 baseline) produced silent return-to-menu
  // with no socket attempt and no memory writes from netwk.bin.
  return enterAreasGameStartPage(sessid);
}

function enterAreasGameStartPage(sessid) {
  // CRS-GAME-START with start_url=/lbs_info — same shape as gameStartPage()
  // but routed to /lbs_info instead of /enterareas. 4 fields exactly.
  const csv = [
    'OK',
    sessid,
    `${BASE_URL}/lbs_info`,
...
function lbsInfoPage(sessid) {
  // PAL CSV-parser is strict on field count: exactly 7 rows for LBS info.
  // Probe with extra HANDLE/AUTH fields (2026-05-14) was negative — kept minimal.
...
function isLbsLikeRequest(pathname, reqBody, contentType) {
  // ANY POST to lbs.jsp or enterareas is an LBS handoff — the game's POST body
  // is sometimes just {connection-id=...} (new-account flow) or full form data.
  // Accept either; resolve missing user-id from session.
...
function handleRequest(req, reqBody) {
  ...
  if (pathname.startsWith('/mhweb/assets/') && req.method === 'GET') {
    ...
    if (!safePath.startsWith(WEB_ASSETS_DIR) || !fs.existsSync(safePath)) {
      return { status: 404, body: Buffer.from('Not found', 'utf8'), contentType: 'text/plain; charset=utf-8' };
    }
    const ext = path.extname(safePath).toLowerCase();
    const contentType = ext === '.png' ? 'image/png' : 'application/octet-stream';
    return { status: 200, body: fs.readFileSync(safePath), contentType };
  }

  if (pathname === '/mhweb/index.jsp') return topPage();
  if (pathname === '/mhweb/create.jsp' && req.method === 'GET') return signupForm(null);
  if (pathname === '/mhweb/login.jsp' && req.method === 'GET') return loginForm(null);
  ...
  // 2026-05-21 Option A: GAME-START's start_url points here; serve real LBS data.
  // Browser may POST (with connection-id body) or GET — handle both, and resolve
  // sessid from form body if present, else from query string.
  ...
  // 2026-05-22: /mhweb/0 — instrumented handler. The browser POSTs here when
  // /enterareas returned CRS-GAME-START with field 3 = "0" (which it should
  // NO LONGER, per the baseline rollback). Kept as a defense-in-depth catch
  // so we always log and respond instead of falling into the CRS-ERROR path.
  // Default reply: minimal CRS-LBS-INFO-GET so the browser doesn't see an
  // explicit error. Override via env MHWEB0_VARIANT for experiment matrix C.
  ...
  if (pathname === '/mhweb/logout.jsp') return gameEndPage();
  ...
  if (pathname === '/mhweb/create.jsp' && req.method === 'POST') {
    ...
    // PAL: username IS the handle. No separate handle field.
    accounts[userid] = {
      userid,
      password,
    ...
    // 2026-05-23: respond with AM-USA-COMP-SIGNUP marker to trigger the
    // natural handle-commit chain at 0x5e6b08. See signupCompletePage().
    const sessid = startSession(userid);
    markPendingOnlineFromLbs({ ip: remoteIp, sessid, userid });
    console.log(`[account] created: ${userid}  handle="${userid}"  sessid=${sessid}  -> AM-USA-COMP-SIGNUP response`);
  ...
  if (pathname === '/mhweb/login.jsp' && req.method === 'POST') {
    ...
    // PAL: username IS the handle — backfill if older account had null
  ...
  if (req.method === 'POST' && isLbsLikeRequest(pathname, reqBody, req.headers['content-type'])) {
    ...
    // user-id may be absent (game only has sessid from CRS-GAME-START for new accounts)
    ...
    // Fire-and-forget PINE inject — make Go Online selectable in PCSX2 testing.
    pineAutoInject(resolvedHandle).catch(() => {});

    // 2026-05-22 E2 dispatch by endpoint:
    //   /enterareas → CRS-GAME-START + start_url=/lbs_info (4-field gameStart shape)
    //   /lbs.jsp    → lbsInfoPage (legacy fallback, unchanged)
    if (pathname === '/mhweb/enterareas' || pathname === '/mhweb/enterareas.jsp') {
...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    const bodyDecoded = (() => {
      try { return parseFormBody(reqBody, req.headers['content-type']); }
      catch { return {}; }
    })();
